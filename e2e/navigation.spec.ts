...
test.describe("처음 온 게스트의 길 찾기", () => {
  test("대시보드 → 첫 토픽 → 문제 화면", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/topics\/complexity$/);
    await expect(page.getByRole("heading", { name: "시간 복잡도 입문", level: 1 })).toBeVisible();
    // Lv1 문제는 열려 있고 Lv2부터는 잠겨 있다
    await expect(page.getByText("Lv1 클리어 후 열려요")).toBeVisible();

    await page.getByRole("link", { name: /배수는 몇 개\?/ }).click();
    await expect(page).toHaveURL(/\/problems\/cx-count-multiples$/);
    await expect(page.getByRole("heading", { name: "배수는 몇 개?" }).first()).toBeVisible();
  });

  test("로드맵에는 입문과 16개 토픽이 있고, 입문과 스택만 열려 있다", async ({ page }) => {
    ...
    const titles = [
      ...
      "힙 (우선순위 큐)",
      "다익스트라 (최단 경로)",
      "그래프 심화 (유니온 파인드·위상 정렬)",
    ];
    for (const [i, title] of titles.entries()) {
      await expect(page.getByText(`${i}. ${title}`, { exact: true })).toBeVisible();
    }
    await expect(page.getByText("시작할 수 있어요", { exact: true })).toHaveCount(2);
    await expect(page.getByText("잠겨 있어요", { exact: true })).toHaveCount(15);
