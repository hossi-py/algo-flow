...
test.describe("처음 온 게스트의 길 찾기", () => {
  ...
  test("로드맵에는 9개 토픽이 있고 스택만 열려 있다", async ({ page }) => {
    await page.goto("/roadmap");
    const titles = ["스택", "큐 / 덱", "재귀 기초", "그래프와 트리 표현", "DFS", "BFS", "백트래킹", "해시", "정렬"];
    ...
    await expect(page.getByText("잠겨 있어요", { exact: true })).toHaveCount(8);
  });
