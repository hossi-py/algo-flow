...
시간 복잡도 입문과 스택부터 DP·그리디·두 포인터·힙·다익스트라·유니온 파인드·시뮬레이션까지, 개념을 그림과 애니메이션으로 이해하고 → "이 문제는 어떤 유형일까?"를 알아보는 연습을 하고 → 브라우저에서 바로 문제를 풀어요. 설치도, 가입도 필요 없어요.
...
- **알고리즘 숲 로드맵**: 입문 + 17개 토픽 × 5단계(개념 이해 → 구현 → 패턴 → 응용 → 실전), 270문제. 앞 토픽의 Lv3을 깨면 다음 토픽이 열려요.
...
| #   | 토픽                                  | 한 줄 소개                                                      |
| --- | ------------------------------------- | --------------------------------------------------------------- |
| 0   | 시간 복잡도 입문                      | 풀기 전에, 입력이 커지면 몇 번 계산할지 먼저 어림해요           |
...
| 12  | 그리디                                | 매 순간 가장 좋아 보이는 것을 고르되, 반례부터 확인해요         |
| 13  | 두 포인터                             | 손가락 두 개를 한 방향으로만 옮기면, 모든 쌍을 보지 않아도 돼요 |
| 14  | 힙 (우선순위 큐)                      | 응급실처럼, 먼저 온 순서가 아니라 가장 급한 것부터 꺼내요       |
| 15  | 다익스트라 (최단 경로)                | 내비게이션처럼, 지금까지 가장 가까운 곳부터 거리를 확정해요     |
| 16  | 그래프 심화 (유니온 파인드·위상 정렬) | 그룹은 대표끼리 합치고, 할 일은 먼저 할 것부터 줄 세워요        |
| 17  | 구현 / 시뮬레이션                     | 규칙을 빠짐없이 그대로 따라 하면, 그게 곧 답이에요              |
