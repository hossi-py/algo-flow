...
export function applyCardRead(
  ...
  const level = cardsCompleted ? checkLevelOne(next, topic, topics, now) : { progress: next, levelCleared: null, unlockedTopic: null };
  return {
    progress: level.progress,
    outcome: { ...NO_CHANGE, xpAwarded, cardsCompleted, levelCleared: level.levelCleared, unlockedTopic: level.unlockedTopic },
  };
}
...
export function applyQuizResult(
  ...
  return {
    progress: level.progress,
    outcome: { ...NO_CHANGE, xpAwarded, quizPassed, levelCleared: level.levelCleared, unlockedTopic: level.unlockedTopic },
  };
}
