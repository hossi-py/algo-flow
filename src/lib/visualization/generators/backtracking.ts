...
export function permutation(input: JsonValue[]): VisualizationStep[] {
  ...
  const state = (highlight?: { itemId: string; tone: HighlightTone }, current?: number) => ({
    ...
    callStack: Array.from({ length: path.length + 1 }, (_, depth): CallFrame => ({
      id: `bt-${depth}`,
  ...
  const backtrack = () => {
    ...
    for (const [i, x] of items.entries()) {
      ...
      rec.push(
        "choose",
        `아직 안 쓴 ${x} 고르기 → path = ${format(path.map((p) => p.value))}`,
        5,
        state({ itemId: item.id, tone: "current" }, i),
      );
      backtrack();
      path.pop();
      used[i] = false;
      rec.push(
        "unchoose",
        `되돌리기: path에서 ${x} 빼고 다음 선택지를 봐요 → path = ${format(path.map((p) => p.value))}`,
        7,
        state(undefined, i),
      );
    }
...
export function subset(input: JsonValue[]): VisualizationStep[] {
  ...
  const state = () => ({
    ...
    callStack: Array.from({ length: current === null ? 0 : current.length + 1 }, (_, depth): CallFrame => ({
      id: `bt-${depth}`,
  ...
  rec.push(
    "done",
    `부분집합은 모두 ${results.length}개 (2^${n} = ${results.length}). 트리의 잎 하나가 부분집합 하나예요.`,
    null,
    state(),
  );
  return rec.steps;
