...
export function factorial(input: JsonValue[]): VisualizationStep[] {
  ...
  const call = (k: number): number => {
    ...
    rec.push("call", `${k} × factorial(${k - 1}) 계산에는 먼저 factorial(${k - 1}) 값이 필요해요 → 재귀 호출`, 3, state());
    const sub = call(k - 1);
    const value = k * sub;
...
export function fibonacci(input: JsonValue[]): VisualizationStep[] {
  ...
  const visit = (node: TreeNode): number => {
    ...
    for (const [index, child] of node.children.entries()) {
      rec.push("call", `fib(${node.k})의 ${index === 0 ? "첫 번째" : "두 번째"} 재귀 호출: fib(${child.k})`, 3, state());
