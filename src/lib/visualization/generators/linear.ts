  ...
  ops.forEach((op, i) => {
    if (op.kind === "push") {
      ...
      rec.push(
        "peek",
        current
          ? `peek(): 맨 위 값은 ${String(current.value)} (꺼내지는 않아요)`
          : "peek(): 비어 있어서 볼 값이 없어요",
        5,
        {
  ...
  rec.push(
    ...
    7,
    {
      stack: linear(
        [...items],
        pointers(),
        items.map((it) => ({ itemId: it.id, tone: "blocked" as const })),
      ),
    ...
    if (op.kind === "enqueue") {
      ...
      rec.push(
        ...
        4,
        {
          ...
          sequence: opsSequence(raw, i, i),
          variables: removed
            ? { 크기: items.length, "꺼낸 값": removed.value }
            : { 크기: 0, 오류: "빈 큐에서 dequeue" },
        },
      );
    } else {
      const front = items[0];
