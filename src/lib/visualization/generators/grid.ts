...
export function gridDfs(input: JsonValue[]): VisualizationStep[] {
  ...
  const dfs = (r: number, c: number): number => {
    ...
    for (const [dr, dc, name] of DIRECTIONS) {
      ...
      if (cells[nr]![nc] !== "1") {
        snap("check", `${name} (${nr}, ${nc}) 칸은 빈 땅 → 건너뛰기`, 8, [r, c], { checking: [nr, nc], checkResult: "blocked" });
        continue;
      }
      if (zone[nr]![nc]) {
...
export function gridBfs(input: JsonValue[]): VisualizationStep[] {
  ...
  while (queue.length > 0) {
    ...
    const [r, c] = (item.value as string).replace(/[()\s]/g, "").split(",").map(Number) as [number, number];
    ...
    if (r === er && c === ec) {
      ...
      snap("done", `도착! 최단 거리는 ${dist[r]![c]}칸이에요. 가까운 칸부터 퍼졌으니 처음 닿은 순간이 가장 짧은 길이에요.`, 4, [r, c]);
      return rec.steps;
    }
    ...
    for (const [dr, dc, name] of DIRECTIONS) {
      ...
      if (nr < 0 || nr >= n || nc < 0 || nc >= m) continue;
      if (cells[nr]![nc] === "#") {
        snap("check", `${name} (${nr}, ${nc}) 칸은 벽 → 건너뛰기`, 6, [r, c], { checking: [nr, nc], checkResult: "blocked" });
        continue;
      }
      if (dist[nr]![nc] !== null) {
