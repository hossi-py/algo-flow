...
export function Stage({ state, wide }: { state: VizState; wide: boolean }) {
  const main = state.graph ? <GraphView snapshot={state.graph} /> : state.grid ? <GridView snapshot={state.grid} /> : null;
  const linear = [
...
export function Player({ steps, pseudocode, resetKey, wide = false, className }: PlayerProps) {
  ...
  return (
    <div
      ...
      tabIndex={0}
      onKeyDown={onKeyDown}
      className={cn("flex min-w-0 flex-col gap-3 rounded-xl outline-none focus-visible:ring-4 focus-visible:ring-ring/40", className)}
    >
