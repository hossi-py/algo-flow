...
export function VisualizationExplorer({ presets, wide = false, className }: VisualizationExplorerProps) {
  ...
  const input = preset ? (customInput[preset.id] ?? preset.input) : [];
  ...
  return (
    <div className={cn("flex min-w-0 flex-col gap-4", className)}>
        ...
        <div role="group" aria-label="시각화 예시 고르기" className="flex flex-wrap gap-1.5">
          {presets.map((p) => (
            <button
              ...
              className={cn(
                "rounded-full border px-3 py-1.5 text-caption font-bold outline-none transition-colors focus-visible:ring-4 focus-visible:ring-ring/40",
                p.id === preset.id
      ...
      {preset.editableInput && (
        <details className="group rounded-lg border bg-card/60 px-4 py-3" open={customInput[preset.id] !== undefined || undefined}>
