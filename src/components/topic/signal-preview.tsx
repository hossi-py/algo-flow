...
export function resolveSignals(signalIds: string[]): PatternSignal[] {
  return signalIds.map(getSignal).filter((s): s is PatternSignal => s !== undefined);
}

/** "이런 표현이 나오면 → 이 알고리즘" + 이유 + 함정 */
export function SignalCard({ signal, className }: { signal: PatternSignal; className?: string }) {
  const strength = STRENGTH_LABEL[signal.strength];
  return (
    <div className={cn("flex flex-col gap-2 rounded-lg border border-border/70 bg-card p-5 shadow-soft", className)}>
...
export function SignalPreview({ signalIds }: { signalIds: string[] }) {
  const signals = resolveSignals(signalIds);
