...
export const STACK_CARDS: ConceptCard[] = [
  {
    ...
    keyPoints: ["마지막에 넣은 값이 먼저 나와요 (LIFO)", "넣고 빼는 곳은 top 하나뿐이에요", "중간 값은 바로 꺼내지 않아요"],
  },
  {
    id: "stack-ops",
    title: "push · pop · peek",
    ...
    body: [
      ...
      "",
      "세 연산 모두 맨 위만 건드리므로 스택 크기와 상관없이 **O(1)**로 빨라요. Python은 리스트, JavaScript는 배열을 그대로 스택으로 쓸 수 있어요.",
    ].join("\n"),
    illustration: "stack-plates",
  ...
  },
  {
    ...
    code: {
      code: {
        python: ["if stack:            # 비어 있지 않을 때만", "    x = stack.pop()", "else:", "    ...              # 비었을 때 처리"].join("\n"),
        javascript: [
...
export const STACK_QUIZ: RecognitionQuestion[] = [
  {
    id: "stack-q1",
    snippet: '문자열 s가 주어질 때, 모든 괄호가 올바르게 열리고 닫혔는지 판단하세요. 예를 들어 "(()())"는 올바르고 ")("는 올바르지 않아요.',
  ...
  },
  {
    ...
    explanation: "새 글자를 볼 때마다 '방금 남아 있는 글자(직전 값)'와 비교해요. 같으면 pop, 다르면 push — 스택으로 한 번에 풀려요.",
  },
  {
    id: "stack-q5",
    ...
    explanation: "'최소 횟수'는 BFS의 강한 신호예요. 뒤에서 배울 토픽이지만, 스택 문제가 아니라는 걸 알아보는 연습이에요.",
  },
];
