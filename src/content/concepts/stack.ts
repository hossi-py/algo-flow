...
export const STACK_CARDS: ConceptCard[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "",
      "세 연산 모두 맨 위만 건드리므로 스택 크기와 상관없이 항상 빨라요(**O(1)**). Python은 리스트, JavaScript는 배열을 그대로 스택으로 쓸 수 있어요. Java는 `Deque<Integer> stack = new ArrayDeque<>()`를 써요.",
    ].join("\n"),
    illustration: "stack-plates",
    ...
    code: {
      code: {
        ...
        java: [
          ...
          "stack.push(3);              // push",
          "stack.push(5);",
          "int top = stack.peek();     // peek → 5",
          "int x = stack.pop();        // pop  → 5",
        ].join("\n"),
      },
    },
  },
  ...
  },
  {
    ...
    body: [
      "비어 있는 스택에서 pop을 하면 Python은 **IndexError**가 나요. JavaScript는 오류 없이 **undefined**를 돌려주기 때문에 더 알아채기 어려워요. Java의 `ArrayDeque`는 **NoSuchElementException**이 나요.",
      "",
    ...
    code: {
      code: {
        ...
        java: [
          "if (!stack.isEmpty()) {     // 비어 있지 않을 때만",
          "    int x = stack.pop();",
          "} else {",
          "    // 비었을 때 처리 (빈 스택에서 pop하면 예외가 나요)",
          "}",
        ].join("\n"),
      },
    },
