...
| 완료 기준 | ☑ 신규 토픽(해시·정렬·이분 탐색·DP)이 동일한 학습 흐름(개념 → 시각화 → 유형 인식 → 문제 → AI)으로 동작 ☑ 주요 사용자 흐름 E2E 통과 — `pnpm test:e2e` 14개(데스크톱 12 · 모바일 2): 대시보드 → 토픽 → 문제 길 찾기, 로드맵 잠금, 404, JS 정답 → XP·해결 수가 새로고침 후에도 유지, 오답의 첫 실패 케이스 표시, 무한 반복 → 시간 초과 후 재실행, Python 문법 오류 → 정답, 언어별 초안 저장, Java(CheerpJ) 정답, 힌트 순서·확인 창·유지, 개념 카드·시각화 스텝·유형 퀴즈, 모바일 가로 스크롤 없음·탭 전환 풀이 |
...
| 진행 | ☑ 관리자 화면 (`/admin`, 읽기 전용) — 운영 현황(합계·최근 14일 추이·최근 에러), 회원 목록(이메일·닉네임 검색, 정렬, 페이지)·상세(토픽 진도, 최근 제출, 배지, 학습 기록, AI 사용량). 권한은 `admins` 테이블 + 요청마다 서버 확인(페이지·조회 함수 모두), 관리자가 아니면 일반 404와 똑같이(탭 제목·robots 포함) 응답. 회원 코드·코치 대화는 보여 주지 않음 |
| 진행 | ☑ 주간 XP 랭킹(`/ranking`, 닉네임만·숨기기 가능) — 서버 재채점은 하지 않기로 하고, 제출 API의 모순 차단 + 이상 기록 탐지(번개 해결·XP 과다·짧은 정답 코드)로 대신. 탐지된 회원은 그 주 랭킹에서 빠지고 관리자 화면에 표시 |
| 진행 | ☑ 해시 토픽 — 개념 카드 5장·유형 인식 퀴즈 5문항·유형 신호 5개(중복 확인·개수 세기·짝 찾기·같은 것끼리 묶기·구간 합), 시각화 3개(버킷과 충돌·개수 세기·두 수의 합, `HashView`), 15문제(Lv1 set · Lv2 개수 세기 · Lv3 짝 찾기 · Lv4 묶기 · Lv5 누적 합/슬라이딩 윈도우)의 Python·JS·Java 모범 답안, 큰 입력으로 O(N²) 풀이는 시간 초과. 백트래킹 Lv3 클리어 후 열림 (총 8토픽 120문제) |
| 진행 | ☑ 정렬 토픽 — 개념 카드 5장·유형 인식 퀴즈 5문항·유형 신호 5개(정렬 기준·정렬 후 이웃·구간·작은 값 범위·뒤바뀐 쌍), 시각화 3개(삽입·병합·계수 정렬, 막대 그림 `BarsView`), 15문제(Lv1 정렬 후 보기 · Lv2 합치기·계수·교환 · Lv3 정렬 기준 · Lv4 구간·두 포인터 · Lv5 뒤바뀐 쌍·비교 규칙·좌표 압축)의 Python·JS·Java 모범 답안. 해시 Lv3 클리어 후 열림 (총 9토픽 135문제) |
| 진행 | ☑ 이분 탐색 토픽 — 개념 카드 5장·유형 인식 퀴즈 5문항·유형 신호 4개(정렬+질문 많음·처음으로 ~ 이상·최대의 최소·엄청 큰 범위), 시각화 3개(정확히 찾기·경계 찾기·답을 이분 탐색, 막대 그림에 lo·mid·hi), 15문제(Lv1 정확히 찾기 · Lv2 경계·범위 개수 · Lv3 답을 이분 탐색 · Lv4 최대의 최소 · Lv5 곱셈표 K번째·회전 배열·LIS)의 Python·JS·Java 모범 답안. 정렬 Lv3 클리어 후 열림 (총 10토픽 150문제) |
| 진행 | ☑ DP 토픽 — 개념 카드 5장·유형 인식 퀴즈 5문항·유형 신호 4개(방법의 수+나머지·반복되는 작은 문제·선택이 이어지는 최대/최소·두 문자열), 시각화 3개(계단·격자 길·LCS, 표 그림 `TableView`), 15문제(Lv1 1차원 · Lv2 격자 · Lv3 동전·배낭 · Lv4 연속 합·LCS·편집 거리 · Lv5 상태 DP·회문 자르기·암호 해독)의 Python·JS·Java 모범 답안. 이분 탐색 Lv3 클리어 후 열림 (총 11토픽 165문제, 준비 중 토픽 없음). AI 문제 생성의 알고리즘 이름 노출 검사에 해시·이분 탐색·DP 추가 |
| 진행 | ☑ 섞어 풀기(`/practice`) — 토픽을 숨기고 본문·제약 조건만 보여 주는 문제를 여러 토픽에서 5·10문제 섞어 내고, 유형을 먼저 고르게 한다. 보기는 정답 + 헷갈리기 쉬운 토픽 2개 + 나머지 1개. 고르면 근거 신호와 풀러 가기 링크, 끝나면 점수와 헷갈린 토픽. 본문에 토픽 이름이 나오는 문제와 Lv1은 제외. 기록은 브라우저에만(XP·랭킹과 무관). 입구: 로드맵 머리말, 대시보드 오늘의 유형 신호 |
| 진행 | ☑ 그리디 토픽 — 개념 카드 5장(반례 카드 포함)·유형 인식 퀴즈 5문항(DP와 가르는 문항 포함)·유형 신호 3개, 시각화 3개(회의 고르기 간트 표·동전 거스름돈·큰 수 만들기 스택), 15문제(Lv1 한 기준 · Lv2 정렬 기준 · Lv3 구간·도달 범위 · Lv4 모자라면 다시·양방향 · Lv5 큰 수·구간 나누기·줄 복원)의 Python·JS·Java 모범 답안. 새 토픽은 기존 사용자의 해제 상태를 바꾸지 않도록 로드맵 끝에 이어 붙임(DP Lv3 클리어 후 열림, 총 12토픽 180문제) |
