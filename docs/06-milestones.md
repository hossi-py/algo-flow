...
| 진행 | ☑ 두 포인터 토픽 — 개념 카드 5장·유형 인식 퀴즈 5문항(해시·누적 합과 가르는 문항 포함)·유형 신호 3개, 시각화 3개(양 끝에서 좁히기·늘였다 줄이는 창·중복 없애기, 막대 그림에 L·R·창), 15문제(Lv1 양 끝·같은 방향 · Lv2 두 줄 함께 · Lv3 고정 창 · Lv4 변하는 창 · Lv5 세 수의 합·모든 재료 창·빗물)의 Python·JS·Java 모범 답안. 그리디 Lv3 클리어 후 열림 (총 13토픽 195문제) |
| 진행 | ☑ 힙 토픽 — 개념 카드 5장(응급실·부모 ≤ 자식·언어별 힙·상위 K와 힙 두 개·언제 쓸까)·유형 인식 퀴즈 5문항(큐와 가르는 문항 포함)·유형 신호 3개, 시각화 3개(넣기·꺼내기·K개 줄 합치기·크기 K 힙, 힙 배열을 트리로 그림), 15문제(Lv1 넣고 꺼내기 · Lv2 꺼내고 다시 넣기 · Lv3 상위 K·여러 줄 · Lv4 조건 순서 처리 · Lv5 중앙값·물 보충·수강 계획)의 Python·JS·Java 모범 답안. 두 포인터 Lv3 클리어 후 열림 (총 14토픽 210문제) |
| 진행 | ☑ 다익스트라 토픽 — 개념 카드 5장(비용이 다른 길·가까운 곳부터 확정·힙으로 구현·변형·언제 쓸까)·유형 인식 퀴즈 5문항(BFS·DP와 가르는 문항 포함)·유형 신호 3개, 시각화 3개(기본·경로 되짚기·격자, 그래프 간선에 가중치 표시와 방향 화살표 추가), 15문제(Lv1 기본 · Lv2 격자·경로 되짚기 · Lv3 뒤집은 그래프·여러 출발점·최댓값 최소화 · Lv4 쿠폰·환승 제한·최단 경로 수 · Lv5 꼭 들를 곳·0/1 비용·최단 경로 위 도로)의 Python·JS·Java 모범 답안. 힙 Lv3 클리어 후 열림 (총 15토픽 225문제) |
| 진행 | ☑ 그래프 심화 토픽 — 개념 카드 5장(유니온 파인드·경로 압축과 크기로 합치기·크루스칼·위상 정렬·언제 쓸까)·유형 인식 퀴즈 5문항(다익스트라·DFS와 가르는 문항 포함)·유형 신호 3개, 시각화 3개(합치고 대표 찾기·크루스칼·칸 알고리즘, 방향 그래프 화살표 활용), 15문제(Lv1 합치고 찾기 · Lv2 그룹 크기·같다/다르다·솟아오르는 섬 · Lv3 최소 신장 트리 · Lv4 위상 정렬·고리·공정표 · Lv5 학기 DP·거꾸로 합치기·좌표 최소 신장 트리)의 Python·JS·Java 모범 답안. 다익스트라 Lv3 클리어 후 열림 (총 16토픽 240문제) |
| 진행 | ☑ 시간 복잡도 입문 토픽 (로드맵 맨 앞 0번, 스택과 함께 처음부터 열림) — 개념 카드 5장(연산 횟수·자주 만나는 복잡도·제한 보고 고르기·줄이는 방법·모든 문제의 첫 단계)·유형 인식 퀴즈 5문항(백트래킹·정렬과 가르는 문항 포함)·유형 신호 3개, 시각화 3개(증가 속도 비교·모든 쌍 vs 한 번 훑기·절반씩 줄이기), 15문제(Lv1 공식 O(1) · Lv2 한 번 훑기·누적 합 · Lv3 절반씩 O(log N) · Lv4 제곱근까지 O(√N) · Lv5 최대 연속 합·에라토스테네스의 체·공평한 분할)의 Python·JS·Java 모범 답안 (총 17토픽 255문제) |
| 진행 | ☑ 구현 / 시뮬레이션 토픽 — 개념 카드 5장(규칙 옮기기·방향 번호와 dr/dc·판 돌리기·시각과 날짜·푸는 순서)·유형 인식 퀴즈 5문항(BFS와 가르는 문항 포함)·유형 신호 3개, 시각화 3개(돌고 걷는 로봇·달팽이 채우기·90도 돌리기), 15문제(Lv1 명령 따라 하기·시각 더하기 · Lv2 창고 로봇·달팽이·판 돌리기 · Lv3 이끼 번식·중력·주사위 · Lv4 대여 요금·문자열 줄이기·날짜 · Lv5 애벌레 게임·청소 로봇·사다리)의 Python·JS·Java 모범 답안. 그래프 심화 Lv3 클리어 후 열림 (총 18토픽 270문제) |
