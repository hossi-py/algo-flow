...
├─────────────────────────────────────────────────────────┤
│ TopicProgressRow: 토픽 18개 가로 스크롤 카드 (진행률 링)           │
└─────────────────────────────────────────────────────────┘
```
...
구불구불한 "숲길" 위에 토픽 노드 18개(입문 포함)가 놓인 형태(데스크톱은 세로 지그재그, 모바일은 세로 일렬).
...
```
  (시간 복잡도 입문)●━━━━┓
   ...
   ...  🔒(백트래킹) ━━ 🔒(해시) ━━ 🔒(정렬) ━━ 🔒(이분 탐색) ━━ 🔒(DP) ━━ 🔒(그리디) ━━ 🔒(두 포인터) ━━ 🔒(힙) ━━ 🔒(다익스트라) ━━ 🔒(그래프 심화) ━━ 🔒(구현)
```
...
│  └─ TopicNode × 18    : 아이콘, 진행률 링, 상태(locked/available/in-progress/mastered)
