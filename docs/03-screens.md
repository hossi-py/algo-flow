...
├─────────────────────────────────────────────────────────┤
│ TopicProgressRow: 토픽 7개 가로 스크롤 카드 (진행률 링)           │
└─────────────────────────────────────────────────────────┘
```
...
구불구불한 "숲길" 위에 토픽 노드 7개가 놓인 형태(데스크톱은 세로 지그재그, 모바일은 세로 일렬).
...
│  └─ TopicNode × 7     : 아이콘, 진행률 링, 상태(locked/available/in-progress/mastered)
│     └─ LevelSteps     : (클릭 시 펼침) Lv1~Lv5 칩, 각 레벨 문제 수/해결 수
├─ LockTooltip          : 잠금 해제 조건
└─ ComingSoonNode       : 해시·정렬·이분 탐색·DP (흐리게, "곧 열려요")
```
...
세 단계를 한 페이지에서 순서대로 진행(상단 스텝 탭). 각 단계 완료 시 다음 단계가 열린다.
...
┌───────────────────┐
│ ← 꽃밭 구역  Lv3   │
│ [문제|코드|시각화]  │ ← MobileTabs (문제 탭 안에 힌트/코치 서브탭)
│                   │
│   (선택된 탭)       │
│                   │
├───────────────────┤
