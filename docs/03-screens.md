...
├─────────────────────────────────────────────────────────┤
│ TopicProgressRow: 토픽 10개 가로 스크롤 카드 (진행률 링)           │
└─────────────────────────────────────────────────────────┘
```
...
구불구불한 "숲길" 위에 토픽 노드 10개가 놓인 형태(데스크톱은 세로 지그재그, 모바일은 세로 일렬).
...
│  └─ TopicNode × 10    : 아이콘, 진행률 링, 상태(locked/available/in-progress/mastered)
│     └─ LevelSteps     : (클릭 시 펼침) Lv1~Lv5 칩, 각 레벨 문제 수/해결 수
├─ LockTooltip          : 잠금 해제 조건
└─ ComingSoonNode       : DP (흐리게, "곧 열려요")
```
