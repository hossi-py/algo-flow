...
├─────────────────────────────────────────────────────────┤
│ TopicProgressRow: 토픽 17개 가로 스크롤 카드 (진행률 링)           │
└─────────────────────────────────────────────────────────┘
```
...
구불구불한 "숲길" 위에 토픽 노드 17개(입문 포함)가 놓인 형태(데스크톱은 세로 지그재그, 모바일은 세로 일렬).
...
│  └─ TopicNode × 17    : 아이콘, 진행률 링, 상태(locked/available/in-progress/mastered)
